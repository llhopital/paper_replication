...
%MM          =  share_imp/100*Y - (varphiM+0.0035) *Y; 
%MM_FF       =  varphiM*Y;
%MM_K          = PTH*XX + C + INV + PG * G - PN * YN  -S * YT - MM_FF - MM; 

MM_TOTAL      =  share_imp/100*Y; 
PTH            = (C + INV + PG * G - PN * YN -MM_TOTAL)/(YT - XX);
...
S_CHANGE = 0;
NIIP = -1.41*Y;
GSaving = PG*GI + rd*S*D/(1+ggamma) - S*D + (RDC-1-ggamma)/(1+ggamma)*S*DC + (R-1-ggamma)/(1+ggamma)*B;
T2 = 0;
DT2 = 0;
